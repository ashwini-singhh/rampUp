...
function SummaryInfo({ responses, interview }: SummaryProps) {
  ...
  useEffect(() => {
    ...
    for (const response of responses) {
      const sentiment = response.analytics?.user_sentiment || response.details?.call_analysis?.user_sentiment;
      ...
      const callCompletion = response.is_ended ? "Complete" : response.details?.call_analysis?.call_completion_rating;
      ...
      const agentTaskCompletion = response.is_ended ? "Complete" : response.details?.call_analysis?.agent_task_completion_rating;
        ...
        <div className="bg-slate-200 rounded-2xl min-h-[120px] p-2 ">
          ...
          <div className="flex flex-col gap-1 my-2 mt-4 mx-2 p-4 rounded-2xl bg-slate-50 shadow-md">
            <ScrollArea className="h-[250px]">
              {(() => {
                const hasTutorMetrics = tableData.some((row) => row.clarityScore !== null && row.clarityScore !== undefined);
                return (
                  <DataTable 
