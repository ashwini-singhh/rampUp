...
function DataTable({ data, interviewId }: DataTableProps) {
  const hasTutorMetrics = React.useMemo(() => data.some(
    (row) => row.clarityScore !== null && row.clarityScore !== undefined,
  ), [data]);

  const [sorting, setSorting] = useState<SortingState>([{ id: hasTutorMetrics ? "clarityScore" : "overallScore", desc: true }]);
  ...
  const nameColumn: ColumnDef<TableData> = {
    accessorKey: "name",
    header: ({ column }) => (
      <Button
        variant="ghost"
  ...
  const summaryColumn: ColumnDef<TableData> = {
    accessorKey: "callSummary",
    header: () => (
      <div className="w-full justify-start font-semibold text-[15px] mb-1 text-black">Summary</div>
    ),
    cell: ({ row }) => {
  ...
  const tutorColumns: ColumnDef<TableData>[] = [
    {
      accessorKey: "clarityScore",
      header: ({ column }) => (
        <Button
          variant="ghost"
          className={`w-full justify-center font-semibold text-[13px] mb-1 ${column.getIsSorted() ? "text-indigo-600" : "text-black"}`}
          onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
        >
  ...
  const legacyColumns: ColumnDef<TableData>[] = [
    {
  ...
  const columns: ColumnDef<TableData>[] = [
    nameColumn,
    ...(hasTutorMetrics ? tutorColumns : legacyColumns),
    summaryColumn,
  ];
