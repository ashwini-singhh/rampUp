...
export async function POST(req: Request) {
  ...
  const isDiySession =
    callDetails.is_ended === true ||
    Array.isArray(callDetails.details?.transcript);
  ...
  if (isDiySession) {
    ...
    const durationInSeconds = Math.max(0, Math.round((Date.now() - new Date(callDetails.created_at).getTime()) / 1000));

    await ResponseService.updateResponse(
      { analytics: result.analytics, is_analysed: true, duration: durationInSeconds },
      body.id,
    );
  ...
  return NextResponse.json(
    { callResponse, analytics: result.analytics },
    { status: 200 },
  );
}
